...
function handles =plot_matrixyy (t, data, t2, data2, opt_strct, leg_arr, colourarr, linesize)
...
for i = 1:size(data,2)
    ...
    if zero_means; plotdata2 = plotdata2 - mean(plotdata2); end
    plotdata = downsample(plotdata, ds);
    if ~plotloglog
        [handles.plot handles.ax1 handles.ax2] = plotyy (t, plotdata + (i-1)*shift, t2, plotdata2 + (i-1)*shift); hold on;
    else
%         % Not yet implemented
%         loglog(t, plotdata, colourarr(i), 'LineWidth',linesize); hold on;
%         if (shift ~= 0)
%             handles.plotlog = loglog(t, plotdata + (i-1)*shift, colourarr(i), 'LineWidth',linesize); hold on;
%         end
    end
end
